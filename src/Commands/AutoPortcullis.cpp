#include "AutoPortcullis.h"
#include "Commands/Delay.h"
#include "../Robotmap.h"
#include "Commands/AutoDriveTalons.h"
#include "Commands/AutoStop.h"
#include "Commands/cmdAngleShooterTrajectorySet.h"


AutoPortcullis::AutoPortcullis()
{
	AddSequential (new Delay (1)); //waits one second
	AddSequential (new cmdAngleShooterTrajectorySet(250));//puts arm down
	AddSequential (new AutoDriveTalons(0.5, 0.5, 0.5, 0.5)); //drives forward at 50% speed
	AddSequential (new Delay(3)); //drives for 3 seconds
	AddSequential (new cmdAngleShooterTrajectorySet(180));//lifts arm up to lift up door
	AddSequential (new AutoDriveTalons(.25,.25,.25,.25));//drives forward at 25% speed
	AddSequential (new Delay(5));//drives for 5 seconds
	AddSequential (new AutoStop());


}
